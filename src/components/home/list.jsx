      ...
      <div class="container px-5 py-12 mx-auto">
        ...
        {loading ? (<div className="mx-10 flex items-center justify-center bg-cover py-10">
          ...
          <div class="w-full md:w-[90%] grid md:grid-cols-3 grid-cols-2">
            {apis.map((api) => (
              <Link target='_blank' to={`https://santech.gitbook.io/docs/${api.name.toLowerCase().replace(/\s/g, '-')}-api`} class="p-4">
                <div class="h-full flex flex-col items-center text-center">
                  <img alt="team" class="flex-shrink-0 shadow-lg rounded-lg w-full h-fit md:h-56 object-cover object-center mb-4" src={api.img} />
                  <div class="w-full">
                    <h2 class="title-font font-semibold text-lg text-gray-900">{api.name} Api</h2>
                    <p class="mb-4 text-sm md:text-base">{api.short_desc}</p>
                  </div>
                </div>
              </Link>
            )).slice(0, 6)}
