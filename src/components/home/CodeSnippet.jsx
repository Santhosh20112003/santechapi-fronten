...
export default function CodeSnippet() {
  ...
  return (
    <div className="py-24 w-full flex flex-col md:flex-row items-center gap-10 px-5">
      ...
      <div className="lg:max-w-lg lg:w-1/2 w-full mx-auto">
        <div className="w-full h-11 rounded-t-lg bg-slate-400 flex items-center space-x-1.5 px-3">
          <span className="w-3 h-3 rounded-full bg-red-400"></span>
          <span className="w-3 h-3 rounded-full bg-yellow-400"></span>
          <span className="w-3 h-3 rounded-full bg-green-400"></span>
        </div>
        <div className="bg-slate-200 flex items-center justify-center w-full h-56 p-6 text-gray-600 border-t-0 rounded-b-lg shadow-lg">
          <pre className="text-md break-all lg:text-lg font-mono whitespace-pre-wrap text-slate-600">
            {`fetch( '${link}', { 
  ...
  .catch(err => console.log(err));`}
        ...
        <div className="w-full h-5 rounded-b-lg bg-slate-400 flex justify-start items-center space-x-1.5 px-3"></div>
      </div>
